...
const BookingSearchBar: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto overflow-hidden rounded-2xl bg-white shadow-[0_12px_40px_hsl(var(--brand)/0.32)]">
      <div className="flex flex-col lg:flex-row lg:items-stretch lg:divide-x lg:divide-gold-border">
        <div className="hidden lg:flex flex-col justify-center px-6 py-5 min-w-[170px] bg-surface">
          <p className="font-playfair text-xl font-semibold leading-tight text-brand">
            Book Online
          </p>
          <p className="mt-1 text-xs text-brand-muted">Guaranteed accommodation</p>
        </div>
        ...
        >
          <PopoverTrigger asChild>
            <button
              type="button"
              className={cn(
                "flex min-w-[210px] flex-1 items-center gap-3 border-b border-gold-border px-5 py-4 text-left transition-colors duration-200 hover:bg-surface lg:border-b-0",
                activeField === "guests" && "bg-surface ring-1 ring-inset ring-gold/40"
              )}
              aria-label="Select guests"
              aria-expanded={activeField === "guests"}
            >
              <Users className="h-5 w-5 shrink-0 text-gold-strong stroke-[1.5]" />
              <div className="min-w-0">
                <p className="mb-0.5 text-[11px] font-medium uppercase tracking-[0.14em] text-brand-muted">
                  Guests
                </p>
                <p className="truncate text-[15px] font-semibold leading-snug text-brand">
                  {guestSummary}
                </p>
              </div>
            </button>
          ...
          <PopoverContent
            className="w-[min(100vw-2rem,22rem)] border-gold-border p-0 shadow-xl rounded-lg overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 duration-200"
            align="start"
            sideOffset={8}
            onOpenAutoFocus={(event) => event.preventDefault()}
          >
            <div className="bg-surface px-5 pt-4 pb-2">
              <p className="font-playfair text-xl font-semibold text-brand">Guests</p>
            </div>
            ...
            <div className="flex justify-end border-t border-gold-border bg-surface px-5 py-3">
        ...
        <Button
          ...
          className={cn(
            "h-14 shrink-0 !rounded-none bg-gold-strong px-10 text-[13px] font-semibold uppercase tracking-[0.16em] text-white shadow-none transition-shadow duration-300 hover:bg-gold-deep hover:brightness-100 lg:h-auto lg:min-h-full lg:self-stretch",
            activeField === "search" && "find-room-pulse ring-2 ring-gold ring-offset-2 ring-offset-white"
          )}
        >
          Find Room
        </Button>
...
const CounterRow: React.FC<CounterRowProps> = ({
  ...
  <div>
    <p className="mb-2 text-sm text-brand-muted">{label}</p>
    <div className="inline-flex items-center overflow-hidden rounded-full border border-gold-border">
      <Button
        type="button"
        variant="subtle"
        size="icon-sm"
        className="rounded-none text-brand hover:bg-surface-elevated"
      ...
      <span className="flex h-10 min-w-[3rem] items-center justify-center border-x border-gold-border px-3 text-sm font-medium text-brand">
        {value}
      </span>
      <Button
        type="button"
  ...
  <Popover open={open} onOpenChange={onOpenChange}>
    <PopoverTrigger asChild>
      <button
        type="button"
        className={cn(
          "flex w-full min-w-[180px] items-center gap-3 border-b border-gold-border px-5 py-4 text-left transition-colors duration-200 hover:bg-surface lg:border-b-0",
          active && "bg-surface ring-1 ring-inset ring-gold/40"
        )}
        aria-label={`Select ${label}`}
        aria-expanded={open}
      >
        <CalendarIcon className="h-5 w-5 shrink-0 text-gold-strong stroke-[1.5]" />
        <div className="min-w-0">
          <p className="mb-0.5 text-[11px] font-medium uppercase tracking-[0.14em] text-brand-muted">
            {label}
          </p>
    ...
    <PopoverContent
      className="w-auto overflow-hidden rounded-lg border-gold-border p-0 shadow-xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 duration-200"
      align="start"
      sideOffset={8}
    >
      <Calendar
